...
import MinimalistOrderCard from '@/components/shop/MinimalistOrderCard';
...
const ShopDashboard: React.FC = () => {
  ...
  // Demo orders with clear bifurcation
  ...
  // Separate orders by type and sort by urgency + time
  const sortOrders = (ordersList: ShopOrder[]) => {
    return ordersList
      ...
      .slice(0, 8); // Show max 8 orders per box
  };

  const uploadedFilesOrders = sortOrders(orders.filter(order => order.orderType === 'uploaded-files'));
  const walkInOrders = sortOrders(orders.filter(order => order.orderType === 'walk-in'));

  const updateOrderStatus = (orderId: string, newStatus: ShopOrder['status']) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cream-50 via-white to-cream-100 font-inter">
      {/* Minimalist Header */}
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-6">
              <div>
                ...
                <p className="text-sm text-neutral-600">Shop Owner Panel</p>
              </div>
